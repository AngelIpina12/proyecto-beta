...
USE [cargoquin]
GO
/****** Object:  StoredProcedure [dbo].[USP_CustomerPortal_INVByEndCustomer]    **/
SET ANSI_NULLS ON
GO
...
-- Description:	Consulta de inventario ingresado el dia de actual , filtrado por coinceidencia de cliente 
-- =============================================

--CREATE PROCEDURE [dbo].[USP_CustomerPortal_INVByEndCustomer]
ALTER PROCEDURE [dbo].[USP_CustomerPortal_INVByEndCustomer]

----WMSPREPROD.WMSPREPRODDBV24.dbo.
----WMSPREPROD.WMSPREPRODDBV24.Security

(@intUserId INT NULL
 ,@strEndCustomer varchar(30) NULL
)
AS
BEGIN 
	...
	CREATE TABLE #tempInvList(
				  ...
				  ,strWayShip VARCHAR(50) NULL
				   ...
				   ,intPartId INT NULL
				   ,strCustomerName VARCHAR(100) NULL
				   ) 
	...
	--- endcustomer
	 --strCustomerName
	  UPDATE #tempInvList	  
	  SET 	strCustomerName  = custrts.Name
	  FROM #tempInvList tbc	  
	  inner join Customers  custrts on custrts.Id = tbc.intCustomer	  
	  WHERE custrts.Id = tbc.intCustomer	  
	 
	 ---
	  DELETE #tempInvList
	  FROM #tempInvList	  
	  WHERE strCustomerName  NOT LIKE '%'+@strEndCustomer +'%'
	  --
	   ...
	   ---
		...
		DROP TABLE #tbHand
		 ...
		 SELECT		  
		  ...
		  GROUP BY   td.strCustomerName ,td.strSKUEndCustomer  , td.strSKUSupplier
		             ,td.intToltalInvPieces , td.strUOM 
