USE [cargoquin]
GO
/****** Object:  StoredProcedure [dbo].[USP_CustomerPortal_RECGetListbyEndCustomDate]     ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[USP_CustomerPortal_RECGetListbyEndCustomDate]
--ALTER PROCEDURE [dbo].[USP_CustomerPortal_RECGetListbyEndCustomDate]
...
BEGIN 
	...
	-------------------
			 ...
			 SELECT 
				 ...
				 --DtmReceivedDate ,intCustomerId
				 ,rec.ReceivingStart , rts.CustomerId
				 -- strEndCustomer  
				 ,custrts.Name
				...
				AND  (custrts.Name LIKE '%' +@strCustomerName  +'%'
				        OR 
					  custrts.Name =ISNULL(@strCustomerName ,custrts.Name)
				     )
			   AND  rts.CustomerId =  ISNULL( @intCustomerId ,rts.CustomerId )
			  Order by rec.Id ASC   
		 ...
		 SELECT    ID  AS 'rowId' 
					...
					,strEndCustomer   AS 'strEndCustomer'
					, intCustomerId as 'intCustomerId'
		 FROM #tempReceivingList
		 ORDER BY DtmReceivedDate 
