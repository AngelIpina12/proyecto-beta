...
USE [cargoquin]
GO
/****** Object:  StoredProcedure [dbo].[USP_CustomerPortal_SHIPTDetailByEndCustom]    **/
SET ANSI_NULLS ON
GO
...
--CREATE PROCEDURE [dbo].[USP_CustomerPortal_SHIPTDetailByEndCustom]
ALTER PROCEDURE [dbo].[USP_CustomerPortal_SHIPTDetailByEndCustom]

----WMSPREPROD.WMSPREPRODDBV24.dbo.
----WMSPREPROD.WMSPREPRODDBV24.Security

(@intUserId INT NULL
,@strEndCustomer varchar(22) NULL
...
BEGIN 
	...
	CREATE TABLE #tempShipM(
				   ...
				   ,strCustomerfolio varchar(300)null 
  ...
  --- 
  	CREATE TABLE #tempShipBS(
				   ...
				   ,strEndCustomer varchar(100) null
				   )
		...
		SELECT 		
		  ...
		  ---------------
		   ...
		   -- INNER JOIN RecTrailerShipments RECS  ON RECS.RecTrailerId = REC.Id
			...
			--
			UPDATE #tempShipBS
			SET strEndCustomer = CU.Name
			FROM #tempShipBS BS
			 INNER JOIN Customers CU  ON BS.intCustomerId  = CU.Id
			WHERE BS.intCustomerId  = CU.Id

			DELETE #tempShipBS 
			FROM #tempShipBS bs
			where bs.strEndCustomer not like '%' + @strEndCustomer + '%'
			---
			---
			--
	  -- insertar detalle 
	    INSERT INTO #tempShipM( 
				  ...
				  bs.intRecId,  bs.intRecStatOut
				   ...
				   SELECT  BS.strTrailerFolio AS 'strTrailerFolio'
						  ...
						  ,BS.strEndCustomer AS 'strEndCustomer'
