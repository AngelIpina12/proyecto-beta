...
USE [cargoquin]
GO
/****** Object:  StoredProcedure [dbo].[USP_CustomerPortal_INVSupplier]    **/
SET ANSI_NULLS ON
GO
...
-- Description:	Consulta de inventario filtradro por supplier 
-- =============================================

CREATE PROCEDURE [dbo].[USP_CustomerPortal_INVSupplier]
--ALTER PROCEDURE [dbo].[USP_CustomerPortal_INVSupplier]

----WMSPREPROD.WMSPREPRODDBV24.dbo.
----WMSPREPROD.WMSPREPRODDBV24.Security

(@intUserId INT NULL
,@strSupplierName VARCHAR(30) NULL
)
AS
BEGIN 
	...
	IF ((@strPartyRoleName ='Admins' ) or (@lstrUserId='FranciscoJCH') or (@intUserId =3025))	
	 BEGIN
			   ...
			   SET  @ldtm_StartRange  = CONVERT(DATETIME , '2023-09-21 00:00' ) 
	...
	CREATE TABLE #tempInvList(
		...
		--
		 ...
		 SELECT inv.Id, RECS.Id ,RECS.Id
		 FROM Inventory inv 
			INNER JOIN RecTrailerShipments RECS  ON RECS.InventoryId = inv.Id
			INNER JOIN RecTrailers REC ON REC.Id =RECS.RecTrailerId
		WHERE inv.EntryDate BETWEEN  @ldtm_StartRange   AND @ldtm_EndRange 
		AND inv.CustomerId= @lintCustomerId  
		GROUP BY inv.Id, RECS.Id ,RECS.Id
		...
		---------------------------------------------------------
		 ...
		 GROUP BY IV.Id , RECS.RecTrailerId, RECS.Id
	--------
	...
	----------------
	   ...
	   WHERE tinv.intInvId = inv.Id
        ...
        -----,strSUpplierName 
		  UPDATE #tempInvList  
		  SET strSUpplierName = sup.Name
		  FROM #tempInvList  IL
		  	 INNER JOIN CustomerSuppliers cusup on cusup.CustomerId = IL.intCustomer
				                                     and  IL.intSupplierId = cusup.SupplierId   
			 INNER JOIN  Suppliers sup on sup.Id =cusup .SupplierId
		  --
		
		DELETE #tempInvList  
		FROM #tempInvList  
		WHERE strSUpplierName NOT LIKE '%'+ @strSupplierName+'%'
			---
	  -- custpro
	  UPDATE #tempInvList	  
	   ...
	   --							)
		...
		--	where tbc.intinvid >0						
		 ...
		 SELECT
		  ...
		  GROUP BY  strSUpplierName, strSKUSupplier ,strUOM ,strWayShip 
		             ,intQty ,intBox ,td.intinvid 
					 ,strSKUEndCustomer
			
		 ORDER BY intinvid  ASC
